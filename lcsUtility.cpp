	...
	while ((ch = fgetc(fin)) != EOF) {
		...
		if (isalpha(ch)) {
			...
			if (!strcmp(name, "startTime")) {
				printf("read startTime ... ");
				double value;
				if (fscanf(fin, "%lf", &value) != 1) lcs::Error("Fail to read \"startTime\"");
				this->startTime = value;
				printf("Done. startTime = %lf\n", value);
				continue;
			}
			if (!strcmp(name, "finishTime")) {
				printf("read finishTime ... ");
				double value;
				if (fscanf(fin, "%lf", &value) != 1) lcs::Error("Fail to read \"finishTime\"");
				this->finishTime = value;
				printf("Done. finishTime = %lf\n", value);
				continue;
			}
...
double lcs::Configure::GetStartTime() const {
	return this->startTime;
}

double lcs::Configure::GetFinishTime() const {
