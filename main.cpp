...
int GetIndex(int x, int y, int z, int yRes, int zRes) {
	return (x * (yRes + 1) + y) * (zRes + 1) + z;
}
...
void CalculateFTLE() {	
	...
	*/
	...
	double *ftle = new double [numOfGridPoints];

	printf("check point 3\n");
	
	double max_ftle = -1e100;
	double min_ftle = 1e100;
	...
	for (int x = 0; x <= xRes; x++)
		for (int y = 0; y <= yRes; y++)
			for (int z = 0; z <= zRes; z++) {
				...
				int idx = GetIndex(x, y, z, yRes, zRes);
				if (initialCellLocations[idx] == -1) continue;
				...
				if (x == 0 || initialCellLocations[GetIndex(x - 1, y, z, yRes, zRes)] == -1) {
					x1_idx = x;
					step_x = 0.0;
				} else {
					x1_idx = x - 1;
				...
				if (x == xRes || initialCellLocations[GetIndex(x + 1, y, z, yRes, zRes)] == -1) {
					x2_idx = x;
					step_x = -step_x;
				} else {
					x2_idx = x + 1;
				...
				if (y == 0 || initialCellLocations[GetIndex(x, y - 1, z, yRes, zRes)] == -1) {
					y1_idx = y;
					step_y = 0.0;
				} else {
					y1_idx = y - 1;
				...
				if (y == yRes || initialCellLocations[GetIndex(x, y + 1, z, yRes, zRes)] == -1) {
					y2_idx = y;
					step_y = -step_y;
				} else {
					y2_idx = y + 1;
				...
				if (z == 0 || initialCellLocations[GetIndex(x, y, z - 1, yRes, zRes)] == -1) {
					z1_idx = z;
					step_z = 0.0;
				} else {
					z1_idx = z - 1;
				...
				if (z == zRes || initialCellLocations[GetIndex(x, y, z + 1, yRes, zRes)] == -1) {
					z2_idx = z;
					step_z = -step_z;
				} else {
					z2_idx = z + 1;
				...
				x1_idx = GetIndex(x1_idx, y, z, yRes, zRes);
				x2_idx = GetIndex(x2_idx, y, z, yRes, zRes);

				y1_idx = GetIndex(x, y1_idx, z, yRes, zRes);
				y2_idx = GetIndex(x, y2_idx, z, yRes, zRes);

				z1_idx = GetIndex(x, y, z1_idx, yRes, zRes);
				z2_idx = GetIndex(x, y, z2_idx, yRes, zRes);

				double J[3][3];  // Jacob dx, dy, dz

				J[0][0] = (flowMap[x2_idx * 3] - flowMap[x1_idx * 3]) / step_x;
				J[0][1] = (flowMap[y2_idx * 3] - flowMap[y1_idx * 3]) / step_y;
				J[0][2] = (flowMap[z2_idx * 3] - flowMap[z1_idx * 3]) / step_z;

				J[1][0] = (flowMap[x2_idx * 3 + 1] - flowMap[x1_idx * 3 + 1]) / step_x;
				J[1][1] = (flowMap[y2_idx * 3 + 1] - flowMap[y1_idx * 3 + 1]) / step_y;
				J[1][2] = (flowMap[z2_idx * 3 + 1] - flowMap[z1_idx * 3 + 1]) / step_z;

				J[2][0] = (flowMap[x2_idx * 3 + 2] - flowMap[x1_idx * 3 + 2]) / step_x;
				J[2][1] = (flowMap[y2_idx * 3 + 2] - flowMap[y1_idx * 3 + 2]) / step_y;
				J[2][2] = (flowMap[z2_idx * 3 + 2] - flowMap[z1_idx * 3 + 2]) / step_z;
				...
				*/
				...
				double *EForVTK[3];
				EForVTK[0] = eigenvectors;
				EForVTK[1] = eigenvectors + 3;
				EForVTK[2] = eigenvectors + 6;
				vtkMath::Jacobi(CForVTK, eigenvalues, EForVTK);
	...
	fprintf(fout, "# vtk DataFile Version 3.0\n");
	fprintf(fout, "FTLE values\n");
	fprintf(fout, "ASCII\n");
	fprintf(fout, "DATASET STRUCTURED_POINTS\n");
	fprintf(fout, "DIMENSIONS %d %d %d\n", xRes + 1, yRes + 1, zRes + 1);
	fprintf(fout, "ORIGIN %lf %lf %lf\n", minX, minY, minZ);
	fprintf(fout, "SPACING %lf %lf %lf\n", dx, dy, dz);
	fprintf(fout, "POINT_DATA %d\n", numOfGridPoints);
	fprintf(fout, "SCALARS ftle double\n");
	...
	for (int z = 0; z <= zRes; z++)
		for (int y = 0; y <= yRes; y++)
			for (int x = 0; x <= xRes; x++) {
				int idx = GetIndex(x, y, z, yRes, zRes);
				if (initialCellLocations[idx] == -1) fprintf(fout, "%lf\n", min_ftle - 1.0);
				else if (ftle[idx] != ftle[idx]) fprintf(fout, "0.0\n");
	...
	grid->SetDimensions(xRes + 1, yRes + 1, zRes + 1);
	grid->SetOrigin(minX, minY, minZ);
	grid->SetSpacing(dx, dy, dz);
	
	grid->SetExtent(0, xRes, 0, yRes, 0, zRes);
	...
	*/
	...
	scalars->SetNumberOfTuples(numOfGridPoints);
	...
	for (int z = 0; z <= zRes; z++)
		for (int y = 0; y <= yRes; y++)
			for (int x = 0; x <= xRes; x++) {
				if (initialCellLocations[GetIndex(x, y, z, yRes, zRes)] == -1)
					scalars->SetTuple1(idx, -1.0);
				else
					scalars->SetTuple1(idx, ftle[GetIndex(x, y, z, yRes, zRes)]);
			}
	*/
